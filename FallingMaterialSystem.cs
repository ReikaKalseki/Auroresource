...
namespace ReikaKalseki.Auroresource {
	...
	public class FallingMaterialSystem {
		...
		internal void tick(float time, float dT) {
			...
			if (countdown) {
				if (currentSpawner)
					countdown.setTime(currentSpawner.numberToSpawn, currentSpawner.timeLeft);
				else
					countdown.holder.SetActive(false);
			}
			if (nextReEntry <= 0) {
				this.scheduleNextReEntry(time);
			}
			else if (time >= nextReEntry && this.isPlayerInValidBiome()) {
				//spawnItem();
				this.queueSpawn();
				this.scheduleNextReEntry(time);
			}
		...
		private Vector3 selectRandomPosition() {
			...
			while (!this.isValidPosition(sel)) {
		...
		public bool isValidPosition(Vector3 sel) {
			return !VanillaBiomes.VOID.isInBiome(sel.setY(-5)) && !this.isCloseToExclusion(sel);
		}

		private bool isCloseToExclusion(Vector3 sel) {
			return Vector3.Distance(sel, floatingIslandCenter) <= floatingIslandRadius || MathUtil.getDistanceToLineSegment(sel, mountainIslandPoint1, mountainIslandPoint2) <= mountainIslandPointRadius || WorldUtil.isInsideAurora2D(sel);
		}
		...
		internal void spawnItem() {
			this.spawnItem(MathUtil.getRandomVectorAround(Vector3.zero, new Vector3(1500, 0, 1500)));
		}
		...
		internal void tickMapRoom(MapRoomFunctionality map) {
			...
			if (map.scanActive && map.typeToScan == fallingMaterialSpawner.TechType && this.hasFinderUpgrade(map)) {
				if (currentSpawner) {
		...
		internal bool hasFinderUpgrade(uGUI_MapRoomScanner gui) {
			return this.hasFinderUpgrade(gui.mapRoom);
		}
		...
		void Update() {
			if (Ocean.main.GetDepthOf(gameObject) > 1) {
				...
				if (!isDestroyed && velocity.magnitude < 0.25) {
					...
					Pickupable pp = this.GetComponentInChildren<Pickupable>();
					pp.transform.SetParent(null);
					gameObject.destroy(false, 3);
					float dist = Vector3.Distance(Player.main.transform.position, transform.position);
					if (Player.main.transform.position.y >= -100 || dist <= 200) {
						double depthvol = MathUtil.linterpolate(-Player.main.transform.position.y, 50, 200, 1, 0, true);
						double distvol = MathUtil.linterpolate(dist, 200, 350, 1, 0, true);
						float vol = (float)Math.Max(depthvol, distvol);
	...
	class FallingMaterialSpawnerTag : MonoBehaviour {
		...
		internal void initialize() {
			timeLeft = UnityEngine.Random.Range(5F, 15F) * 60 * AuroresourceMod.config.getFloat(ARConfig.ConfigEntries.REENTRY_WARNING);
